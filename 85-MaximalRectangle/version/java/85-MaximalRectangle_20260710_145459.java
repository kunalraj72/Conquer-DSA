// Last updated: 10/07/2026, 14:54:59
1class Solution {
2
...
72
73
74    public int maximalRectangle(char[][] matrix) {
75        int n = matrix.length;
76        int m = matrix[0].length;
77        int[][] intMatrix = new int[n][m];
78
79        for (int i = 0; i < n; i++) {
80            for (int j = 0; j < m; j++) {
81                
82                intMatrix[i][j] = matrix[i][j] == '1' ?  1 : 0;
83            }
84        }
85
86        int area = largestRectangleArea(intMatrix[0]);
87        for(int i = 1; i < n; i++){
88            for(int j = 0; j < m; j++){
89              if(intMatrix[i][j] != 0) {
90                intMatrix[i][j] = intMatrix[i][j] + intMatrix[i-1][j];
91              }else{
92                intMatrix[i][j] = 0;
93              }
94            }
95            area = Math.max(area, largestRectangleArea(intMatrix[i]));
96        }
97        return area;
98    }
99}